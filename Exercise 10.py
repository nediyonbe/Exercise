superset_candidate = set(input().split())
N = int(input())
checklist = []
for i in range(N):
    tiny_set = set(input().split())
    if tiny_set.issubset(superset_candidate) and len(superset_candidate) > len(tiny_set):
        checklist.append('ok')
    else:
        checklist.append('nok')
print(checklist.count('nok') == 0) # gives true is the initial set is a superset with 1 additional item
